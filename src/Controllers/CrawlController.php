...
namespace Nguonc\Crawler\NguoncCrawler\Controllers;
...
use Nguonc\Crawler\NguoncCrawler\Crawler;
        ...
        try {
            ...
            foreach ($request['link'] as $link) {
                if (preg_match('/(.*?)(\/phim\/)(.*?)/', $link)) {
                    $link = sprintf('%s/phim/%s', config('nguonc_crawler.domain', 'https://phim.nguonc.com'), explode('phim/', $link)[1]);
    ...
    public function showCrawlPage(Request $request)
    {
        ...
        try {
            $categories = Cache::remember('nguonc_categories', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/the-loai', config('nguonc_crawler.domain', 'https://phim.nguonc.com'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });

            $regions = Cache::remember('nguonc_regions', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/quoc-gia', config('nguonc_crawler.domain', 'https://phim.nguonc.com'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });
        } catch (\Throwable $th) {
        ...
        return view('nguonc-crawler::crawl', compact('fields', 'regions', 'categories'));
    }

    public function crawl(Request $request)
    {
        $pattern = sprintf('%s/phim/{slug}', config('nguonc_crawler.domain', 'https://phim.nguonc.com'));
        try {
